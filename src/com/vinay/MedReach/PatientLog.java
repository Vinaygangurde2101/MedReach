...
public class PatientLog extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			PreparedStatement ps = con.prepareStatement("select * from patientdetails where pemail = ? and  ppassword = ?");
			...
			if (rs.next())
			{
				User.setPid(rs.getString(1));
				User.setPemail(rs.getString(5));
				response.sendRedirect("patientDash.html");
			}else {
